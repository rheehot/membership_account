...
import { getData } from './dataExchange.js';
...
const validator = {
  ...
  registerEvent(target, validObj, event = 'change', tag) {
    target.addEventListener(event, async (e) => {
      ...
      const valid = await validObj.validate(input);
...
const showMsg = (validObj, validResult) => {
  ...
  msgField.style.color = validResult.status !== 'good' ? '#f00' : '#37b24d';
};
...
const validID = {
  ...
  async checkDuplication(id) {
    const response = await getData(`/api/users/id/${id}`)
      .then(() => {
        return false;
      })
      .catch(() => {
